...
on_render_menu (function ()
    ...
    if not menu.main_tree:push("Sorcerer: Smoke Edition") then
        return;
    end;
...
on_update(function ()
    ...
    if spells.ball.logics(target_selector_data)then
        cast_end_time = current_time + 0.2;
        return;
    end;
    ...
    -- Updated spell rotation logic for Lightning Spear build
    local spell_priority = require("spell_priority")

    for _, spell_name in ipairs(spell_priority) do
        local spell = spells[spell_name]
        if spell then
            if spell_name == "ball" then
                -- Cast Ball Lightning 4 times if Crackling Energies < 8
                if my_utility.get_crackling_energies() < 8 then
                    if spell.logics(target_selector_data) then
                        cast_end_time = current_time + 0.2;
                        return;
                    end
                end
            elseif spell_name == "spear" then
                -- Cast Lightning Spear if Crackling Energies >= 8
                if my_utility.get_crackling_energies() >= 8 then
            ...
            elseif spell_name == "ice_blade" or spell_name == "fireball" or spell_name == "ice_armor" or spell_name == "flame_shield" then
                if spell.logics(best_target) then
                    cast_end_time = current_time + 0.2;
                    return;
                end
...
console.print("Lua Plugin - Sorcerer Smoke Edition - V1");